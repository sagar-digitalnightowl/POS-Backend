...
routes.addDiscount = async(req,res)=>{
    ...
    res.status(201).json({message: "Delivery added successfully",result: newDiscount});
    }catch (error) {
...
routes.getAllDiscount = async (req,res)=>{
    try{
        const allDiscount = await discountModel.find().populate("brand","name")

        if(!allDiscount || allDiscount == 0){
            res.status(400).json({error:"No Discount Found"})
        }
        return res.status(200).json({result:allDiscount, message:"Discount data retrived successfully"})
    }catch (error) {
    console.log("error = ", error);
    res.status(500).json({ error: "Something Went wrong" });
  }
...
routes.getDiscountById = async(req,res)=>{
    try{
        ...
        const discount = await discountModel.findById(discountId).populate("brand","name")
        
        if(!discount){
            res.status(400).json({error:`Discount is not found with Id ${discountId}`})
        }
        return res.status(200).json({result:discount, message:"Discount retrived successfully"})
    }catch(error){
        console.log("error = ", error);
        res.status(500).json({ error: "Something Went wrong" });
    }
...
routes.updateDiscountById = async(req,res)=>{
    try{
        ...
        // }
        const discount = await discountModel.findByIdAndUpdate(discountId,req.body,{new:true})
