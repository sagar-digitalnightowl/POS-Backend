...
routes.addDepartment = async(req,res)=>{
    try {
        ...
        if(!data.department || !data.departmentID){
            return res.status(400).json({error:"Department and Department Id are required"})
        }
        const department = await departmentModel.create(data)

        return res.status(200).json({result:department,message:"Department added Successfully"})
    } catch (error) {
...
routes.getAllDepartment = async(req,res)=>{
    try {
        ...
        return res.status(200).json({result:allDepartment,message:"All Department fetched successfully"})
    } catch (error) {
        console.log("error = ", error);
        res.status(500).json({ error: "Something Went Wrong" });
    }
...
routes.getDepartmentById = async(req,res)=>{
    try {
        ...
        if(!department){
            return res.status(404).json({error:`Department is not found with Id ${id}`})
        }
        return res.status(200).json({result:department,message:'Department fetch Successfully'})
    } catch (error) {
        console.log("error = ", error);
        res.status(500).json({ error: "Something Went Wrong" });
    }
...
routes.deleteDepartmentById = async(req,res)=>{
    try{
        ...
        if(!department)
            return res.status(404).json({error:`Department is not found with Id ${id}`})
         return res.status(200).json({result:department,message:"Department deleted successfully"})  
       }catch(error){
